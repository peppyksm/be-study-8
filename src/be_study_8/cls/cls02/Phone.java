...
//	}
	...
	Phone(String model, int price) { //String int 순서로 2개
...
//	위에 이미 String int를 받는 생성자가 있으므로 같은 타입 같은 순서 생성자는 만들 수 없음
	...
	Phone(int battery){  //변수 한개 받는 생성자는 있지만 타입이 달라 생성 가능
		this.battery = battery;
	}
	
	Phone(int battery, String model){   //타입, 갯수가 같아도 순서 다르면 생성 가능
		this.battery = battery;
		this.model = model;
		this.price = 0;
	}
	
	Phone(String model, int price, int battery) { //String int int 순서로 3개
	...
	Phone(int battery, String model, int price){ //위처럼 3개 받지만 순서 달라 생성 가능
		this.model = model;
		this.price = price;
		this.battery = battery;
	}
